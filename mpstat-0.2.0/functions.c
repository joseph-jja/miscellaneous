...
void
getfaults (unsigned int *majorflt, unsigned int *minorflt, int maxnumcpus, int *cpurealid)
{
  ...
  *minorflt = 0;
  ...
  while ((ent = readdir (proc))) {
    if (isdigit (ent->d_name[0])) {	/*just to weed out the obvious junk */
      ...
      if (((fd = open (filename, O_RDONLY, 0)) != -1) && (filename != NULL) && (minortotcpu != NULL) && (majortotcpu != NULL)) {	/*this weeds the rest */
	read (fd, buff, BUFFSIZE - 1);
#ifdef __SMP__
	sscanf (buff,
		"%*d %*s %*c %*d %*d %*d %*d %*d %*u %u %*u %u %*u %*d %*d %*u %*u %*d %*d %*u %*u %*d %*u %u %*u %*u %*u %*u %*u %*u %*u %*u %*u %*u %*u %*d %*d %*d %d",
		&minors, &majors, &size, &cproc);
#else
	sscanf (buff,
		"%*d %*s %*c %*d %*d %*d %*d %*d %*u %u %*u %u %*u %*d %*d %*d %*d %*d %*d %*u %*u %*d %*u %u %*u %*u %*u %*u %*u %*u %*u %*u ",
		&minors, &majors, &size);
