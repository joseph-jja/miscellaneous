...
fn write_time() {
    if let Ok(now) = OffsetDateTime::now_local() {
        ...
        let hour_big: u16 = get_offset(0, LEFT_OFFSET);
        let hour_small: u16 = get_offset(1, LEFT_OFFSET);

        let minute_big: u16 = 5 + get_offset(2, LEFT_OFFSET);
        let minute_small: u16 = 5 + get_offset(3, LEFT_OFFSET);

        let mut ampm: &str = "am";

        if hour < 10 {
            number::write(hour_big, TOP_OFFSET, 0);
            number::write(hour_small, TOP_OFFSET, hour);
        } else if hour > 12 {
            ampm = "pm";
            let twelve_hour: u16 = hour - 12;
            let hour_hundred: u16 = twelve_hour / 10;
            let hour_ten: u16 = twelve_hour - hour_hundred * 10;
            number::write(hour_big, TOP_OFFSET, hour_hundred);
            number::write(hour_small, TOP_OFFSET, hour_ten);
        } else {
            ampm = "pm";
        ...
        // 11 is the width
        write_colon(hour_small + 11 + 2 + LEFT_OFFSET, TOP_OFFSET + 5);
        write_colon(hour_small + 11 + 2 + LEFT_OFFSET, TOP_OFFSET + 10);

        if minute < 10 {
            number::write(minute_big, TOP_OFFSET, 0);
            number::write(minute_small, TOP_OFFSET, minute);
        } else {
            let minute_hundred: u16 = minute / 10;
            let minute_ten: u16 = minute - minute_hundred * 10;
            number::write(minute_big, TOP_OFFSET, minute_hundred);
        ...
        write_text_at(right_offset, TOP_OFFSET, &formatted_date.as_str());
        write_text_at(right_offset, TOP_OFFSET + 2, &ampm);
    }
}
